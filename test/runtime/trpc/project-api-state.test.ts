...
function createStateOpsHarness(
	summary = createSummary("task-1"),
...
describe("createStateOps.saveState", () => {
	...
	it("deduplicates automatic title requests across request-scoped state ops", async () => {
		const board = createUntitledBoard();
		stateMocks.saveProjectState.mockResolvedValue(createSavedState(board));
		const deferredTitle = createDeferred<string | null>();
		titleMocks.generateTaskTitle.mockReturnValue(deferredTitle.promise);
		const automaticTitleGeneration = new AutomaticTitleGenerationCoordinator();
		const first = createStateOpsHarness(createSummary("task-1"), automaticTitleGeneration);
		const second = createStateOpsHarness(createSummary("task-1"), automaticTitleGeneration);
		const projectScope = { projectId: "project-a", projectPath: "/tmp/project-a" };

		await Promise.all([
			first.stateOps.saveState(projectScope, { board, expectedRevision: 1 }),
			second.stateOps.saveState(projectScope, { board, expectedRevision: 2 }),
		]);

		expect(titleMocks.generateTaskTitle).toHaveBeenCalledTimes(1);
		deferredTitle.resolve("Generated Title");
		await vi.waitFor(() => {
			expect(first.applyEffects).toHaveBeenCalledWith(
				createTaskTitleUpdatedEffects({
					projectId: "project-a",
					taskId: "task-1",
					title: "Generated Title",
