...
describe.sequential("git history runtime", () => {
	...
	it("returns rename metadata for rename-only commits", async () => {
		const { path: repoPath, cleanup } = createTempDir("quarterdeck-git-history-rename-");
		try {
			initRepository(repoPath);
	...
	it("discards tracked, staged, and untracked working copy changes", async () => {
		const { path: repoPath, cleanup } = createTempDir("quarterdeck-git-history-discard-");
		try {
			initRepository(repoPath);
	...
	it("returns correct UTF-8 paths for non-ASCII filenames", async () => {
		const { path: repoPath, cleanup } = createTempDir("quarterdeck-git-history-nonascii-");
		try {
			initRepository(repoPath);
			const dirName = "提出書類";
	...
	it("reads ahead and behind counts from tracked branches", { timeout: 15_000 }, async () => {
		const { path: sandboxRoot, cleanup } = createTempDir("quarterdeck-git-history-refs-");
		try {
