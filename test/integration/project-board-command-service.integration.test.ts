	...
	it("persists title provenance and fences refreshes against manual ownership and superseded work", async () => {
		await withTemporaryHome(async () => {
			...
			try {
				...
				const mismatchedProvenance = await service.setGeneratedTaskTitle(
					scope,
					"task-a",
					100,
					"Broader objective",
				...
				expect(mismatchedProvenance.changed).toBe(false);
				expect(mismatchedProvenance.state.revision).toBe(created.state.revision);

				// Explicit regeneration records generated provenance even if the wording stays the same.
				const sameTitleRegeneration = await service.setGeneratedTaskTitle(
					scope,
					"task-a",
					100,
					"Opening request",
				...
				expect(sameTitleRegeneration.changed).toBe(true);
				...
				expect(unchanged.state.revision).toBe(sameTitleRegeneration.state.revision);
