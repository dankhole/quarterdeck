...
describe.sequential("ProjectBoardCommandService integration", () => {
	...
	it("persists title provenance and fences refreshes against manual ownership and superseded work", async () => {
		await withTemporaryHome(async () => {
			const { path: sandboxRoot, cleanup } = createTempDir("quarterdeck-title-provenance-");
			try {
				const projectPath = join(sandboxRoot, "project-a");
				mkdirSync(projectPath, { recursive: true });
				initGitRepository(projectPath);
				...
				const automatic = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Broader objective", 200, {
					expectedTitle: "Opening request",
					expectedAutoGenerated: true,
				});
				expect(automatic.changed).toBe(false);
				expect(automatic.state.revision).toBe(created.state.revision);

				// Explicit regeneration opts a legacy/manual title into future refreshes, even if the wording stays the same.
				const optedIn = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Opening request", 200, {
					expectedTitle: "Opening request",
					expectedAutoGenerated: false,
				});
				expect(optedIn.changed).toBe(true);
				expect(findCardInBoard((await loadProjectState(projectPath)).board, "task-a")?.titleAutoGenerated).toBe(
					true,
				);
				const unchanged = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Opening request", 300, {
					expectedTitle: "Opening request",
					expectedAutoGenerated: true,
				});
				expect(unchanged.changed).toBe(false);
				expect(unchanged.state.revision).toBe(optedIn.state.revision);
				const superseded = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Outdated direction", 300, {
					expectedTitle: "Opening request",
					expectedAutoGenerated: true,
					isCurrent: () => false,
				});
				...
				const refreshed = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Broader objective", 300, {
					expectedTitle: "Opening request",
					expectedAutoGenerated: true,
				});
				expect(refreshed.changed).toBe(true);
				expect(findCardInBoard(refreshed.state.board, "task-a")).toMatchObject({
					title: "Broader objective",
					titleAutoGenerated: true,
				});

				// Delay an already-started refresh until a same-wording manual rename has committed.
				let release: () => void = () => {};
				const waiting = new Promise<void>((resolve) => {
					release = resolve;
				});
				const delayedService = new ProjectBoardCommandService({
					getAuthoritativeSessions: async () => {
						await waiting;
						return {};
					},
				});
				const pending = delayedService.setGeneratedTaskTitle(scope, "task-a", 100, "Late title", 500, {
					expectedTitle: "Broader objective",
					expectedAutoGenerated: true,
				});
				const manual = await service.execute(scope, {
					commandId: "manual-same-title",
					expectedRevision: refreshed.state.revision,
					command: { kind: "patch_task", taskId: "task-a", title: "Broader objective", updatedAt: 400 },
				});
				release();
				const stale = await pending;
				expect(stale.changed).toBe(false);
				expect(stale.state.revision).toBe(manual.state.revision);
				expect(findCardInBoard((await loadProjectState(projectPath)).board, "task-a")).toMatchObject({
					title: "Broader objective",
					titleAutoGenerated: false,
				});
				const cleared = await service.execute(scope, {
					commandId: "manual-clear-title",
					expectedRevision: manual.state.revision,
					command: { kind: "patch_task", taskId: "task-a", title: null, updatedAt: 600 },
				});
				const lateInitial = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Initial title", 700);
				expect(lateInitial.changed).toBe(false);
				expect(lateInitial.state.revision).toBe(cleared.state.revision);
				const regenerated = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Restored title", 800, {
					expectedTitle: null,
					expectedAutoGenerated: false,
				});
				const trashed = await service.execute(scope, {
					commandId: "trash-before-refresh",
					expectedRevision: regenerated.state.revision,
					command: { kind: "move_task", taskId: "task-a", targetColumnId: "trash", updatedAt: 900 },
				});
				const afterTrash = await service.setGeneratedTaskTitle(scope, "task-a", 100, "Late refresh", 1000, {
					expectedTitle: "Restored title",
					expectedAutoGenerated: true,
					isCurrent: (lockedBoard) =>
						!lockedBoard.columns.some(
							(column) => column.id === "trash" && column.cards.some((card) => card.id === "task-a"),
						),
				});
				expect(afterTrash.changed).toBe(false);
				expect(afterTrash.state.revision).toBe(trashed.state.revision);
			} finally {
				cleanup();
			}
		});
