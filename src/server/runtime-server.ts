...
export async function createRuntimeServer(deps: CreateRuntimeServerDependencies): Promise<RuntimeServer> {
	...
	const titleRefresh = createAutomaticTaskTitleRefreshListener({
		automaticTitleGeneration,
		resolveProjectScope: (projectId) => {
			const projectPath = deps.projectRegistry.getProjectPathById(projectId);
			return projectPath ? { projectId, projectPath } : null;
		},
		regenerateTaskTitle: taskTitles.regenerate,
	});
	const disposeTitleRefreshListener = deps.runtimeStateHub.subscribeToTaskReviewReady(
		titleRefresh.onTaskReadyForReview,
	);
