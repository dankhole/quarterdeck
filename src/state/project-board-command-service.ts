...
export class ProjectBoardCommandService {
	...
	async setGeneratedTaskTitle(
		...
		return await this.executeInternalMutation(scope, sessions, (board) => {
			...
			if (
				!card ||
				card.createdAt !== taskCreatedAt ||
				card.title !== expectedTitle ||
				(options.expectedAutoGenerated !== undefined &&
					(card.titleAutoGenerated === true) !== options.expectedAutoGenerated) ||
				(options.expectedAutoGenerated === undefined && card.titleAutoGenerated === false) ||
				(options.isCurrent && !options.isCurrent(board))
			) {
				return { board, changed: false };
			}
			const normalizedTitle = title.trim();
			if (!normalizedTitle || (card.title === normalizedTitle && card.titleAutoGenerated === true)) {
				return { board, changed: false };
			}
			return {
				board: {
					...
					columns: board.columns.map((column) => ({
						...
						cards: column.cards.map((candidate) =>
							candidate === card
								? { ...candidate, title: normalizedTitle, titleAutoGenerated: true, updatedAt }
								: candidate,
						),
					})),
				},
