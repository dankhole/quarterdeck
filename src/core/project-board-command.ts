...
function areBoardCardsEqual(first: RuntimeBoardCard, second: RuntimeBoardCard): boolean {
	return (
		first.id === second.id &&
		first.unstarted === second.unstarted &&
		first.title === second.title &&
		first.titleAutoGenerated === second.titleAutoGenerated &&
	...
	switch (command.kind) {
		...
		case "move_task": {
			...
			if (
				...
				(findCardInBoard(board, command.taskId)?.unstarted === true) !== command.expectedUnstarted
			) {
				return { board, changed: false };
			}
