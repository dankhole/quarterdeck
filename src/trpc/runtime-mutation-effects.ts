...
export function createTaskWorkingDirectoryUpdatedEffects(input: {
	projectId: string;
	taskId: string;
	workingDirectory: string;
	useWorktree: boolean;
...
function getEffectKey(effect: RuntimeMutationEffect): string {
	switch (effect.type) {
		...
		case "task_working_directory_updated":
			return `${effect.type}:${effect.projectId}:${effect.taskId}:${effect.workingDirectory}:${effect.useWorktree ? "worktree" : "shared"}`;
	...
	for (const effect of dedupeRuntimeMutationEffects(effects)) {
		switch (effect.type) {
			...
			case "task_title_updated":
				...
				break;
			case "task_working_directory_updated":
				requireBroadcasterMethod(broadcaster, "broadcastTaskWorkingDirectoryUpdated")(
					effect.projectId,
					effect.taskId,
					effect.workingDirectory,
					effect.useWorktree,
				);
				break;
			case "task_base_ref_updated":
