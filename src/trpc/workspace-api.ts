	...
	return {
		...
		loadChanges: async (workspaceScope, input) => {
			...
			if (normalizedInput.mode === "last_turn") {
				const terminalManager = await deps.terminals.ensureTerminalManagerForWorkspace(
		...
		loadState: async (workspaceScope) => {
			return await deps.data.buildWorkspaceStateSnapshot(workspaceScope.workspaceId, workspaceScope.workspacePath);
		},

		notifyStateUpdated: async (workspaceScope) => {
			broadcastStateUpdate(workspaceScope);
			void deps.broadcaster.broadcastRuntimeProjectsUpdated(workspaceScope.workspaceId);
			return { ok: true };
		},
		...
		saveState: async (workspaceScope, input) => {
			try {
				...
				const generateTitle = async (card: (typeof untitledCards)[number]) => {
					...
					deps.broadcaster.broadcastTaskTitleUpdated(workspaceScope.workspaceId, card.id, title, {
						autoGenerated: true,
					});
				};
		...
		notifyTaskTitleUpdated: (workspaceScope, taskId, title) => {
			deps.broadcaster.broadcastTaskTitleUpdated(workspaceScope.workspaceId, taskId, title);
		},

		setTaskDisplaySummary: async (workspaceScope, taskId, text, generatedAt) => {
			const manager = await deps.terminals.ensureTerminalManagerForWorkspace(
				workspaceScope.workspaceId,
				workspaceScope.workspacePath,
			);
		...
		setFocusedTask: (workspaceScope, taskId) => {
			deps.broadcaster.setFocusedTask(workspaceScope.workspaceId, taskId);
		},
