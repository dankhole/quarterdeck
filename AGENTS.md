...
- The resume-failure fallback in task session exit handling must not run after an explicit stop (`suppressAutoRestartOnExit` / auto-restart reason `suppressed`). If a user trashes a resumed Codex task, the stopped resume process can otherwise schedule a fresh non-resume start, which clears the stored `resumeSessionId` before the real untrash resume arrives and forces a `codex resume --last` fallback. Keep the fallback for clean startup-resume exits: startup resume can rely on a fresh prompt after a `codex resume`/`--continue` process exits 0 without leaving an interactive session. Do **not** use that fallback for non-zero resume exits; preserve the failed resume output and stored `resumeSessionId` instead of replacing the terminal with a blank fresh prompt.
