...
- Radix dropdowns nested inside Radix dialogs need both a dialog-local portal container and non-modal dropdown behavior (`modal={false}`). Portal placement alone can keep the menu DOM under the dialog while the nested menu still hides or dismisses dialog content through Radix modal layering. The task harness selector in the new task dialog is the reference pattern.
