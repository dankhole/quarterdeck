...
- The resume-failure fallback in task session exit handling must not run after an explicit stop (`suppressAutoRestartOnExit` / auto-restart reason `suppressed`). If a user trashes a resumed Codex task, the stopped resume process can otherwise schedule a fresh non-resume start, which clears the stored `resumeSessionId` before the real untrash resume arrives and forces a `codex resume --last` fallback. Keep the fallback for clean startup-resume exits: startup resume can rely on a fresh prompt after a `codex resume`/`--continue` process exits 0 without leaving an interactive session. Do **not** use that fallback for non-zero resume exits. Preserve the failed resume output in the terminal, but if `codex resume <stored-id>` failed non-zero, clear that stored id, warn/toast, and let the next explicit restart fall back to `codex resume --last` instead of retrying the same missing id forever.
