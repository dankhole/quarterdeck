...
- Codex native hook state has source-specific edges: Codex permission approval can resume through `PostToolUse` without a separate permission-resolved hook, so the permission transition guard must allow Codex `PostToolUse` back to running. Codex 0.142.5+ dispatches root `Stop` and `SubagentStop` separately; Quarterdeck installs the review transition only for root `Stop`. Preserve that event boundary instead of inferring root/subagent identity from missing `finalMessage` or `conversationSummaryText`.
