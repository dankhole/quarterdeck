...
**Backend — git stash functions**: Seven functions added to `git-sync.ts` after `discardSingleFile()`: `stashPush` (validates paths via `validateGitPath`, always includes `--include-untracked`, optional `-m` message), `stashList` (parses `--format=%gd%x1f%gs%x1f%ci` with regex extraction for index/branch/message), `stashPop`/`stashApply` (conflict detection via combined `output` field — git writes CONFLICT to stdout not stderr), `stashDrop`, `stashShow` (diff via `git stash show -p`), `stashCount` (`git --no-optional-locks stash list` line count for safe polling). Six tRPC endpoints in `workspace-api.ts` following the `commitSelectedFiles` handler pattern — mutations broadcast `broadcastRuntimeWorkspaceStateUpdated`, queries don't. Pop/apply broadcasts gated on `response.ok || response.conflicted` (invalid-index failures don't broadcast).

**Backend — metadata polling**: `homeStashCount` added to `runtimeWorkspaceMetadataSchema` and `CachedHomeGitMetadata`. In `loadHomeGitMetadata()`, `stashCount` runs in `Promise.all` with `probeGitWorkspaceState`. Stash count compared independently of `stateToken` — stash-only mutations (drop on clean tree) don't change git status output, so stash count must be diffed separately. When only stash count changes, returns updated metadata with new `stateVersion`. Not added to `runtimeTaskWorkspaceMetadataSchema` — stash is shared across worktrees (git stash is repo-level), so `homeStashCount` is the single source of truth.

**Backend — dirtyTree structured field**: Optional `dirtyTree: z.boolean()` added to both `runtimeGitSyncResponseSchema` and `runtimeGitCheckoutResponseSchema`. Checkout: regex `/(?:local changes|uncommitted changes|overwritten by checkout)/i` on stderr. Pull: set explicitly in the existing `changedFiles > 0` early return. Enables frontend to offer stash-and-retry without fragile string matching.

**Frontend — commit panel**: `use-commit-panel.ts` extended with `stashChanges()` (collects selected paths from existing checkbox state — empty array for all), `isStashing` (added to `isMutating` for polling suppression), `stashMessage`/`setStashMessage`. `commit-panel.tsx` adds Stash button between Commit and Discard All with collapsible message input (MessageSquare icon toggle). Button disabled when no files or stashing in progress.

**Frontend — stash list**: New `use-stash-list.ts` hook manages expand/collapse state, on-demand fetching (triggered by expand and by `homeStashCount` changes from metadata store), and action callbacks (pop/apply/drop/showDiff) with toast feedback. New `stash-list-section.tsx` component: Radix `Collapsible.Root`, badge count header, entry rows with context menu (Radix ContextMenu), AlertDialog drop confirmation with `useRef` flag pattern, diff preview via Radix Popover. `useHomeStashCount()` selector hook added to `workspace-metadata-store.ts`.

**Frontend — stash-and-retry checkout**: `checkout-confirmation-dialog.tsx` gains "Stash & Switch" button in `dirty_warning` variant (`variant="primary"` blue accent). `use-branch-actions.ts` adds `handleStashAndCheckout()` — stash push then `performCheckout`, wired to all three dialog render sites (App.tsx home, topbar, card-detail-view task). `use-git-actions.ts` `switchHomeBranch` detects `payload.dirtyTree` and shows 12s toast with "Stash & Switch" action via `showAppToast` action prop (added to `app-toaster.ts`).

**Frontend — stash-and-retry pull**: `git-action-error-dialog.tsx` gains optional "Stash & Pull" button when `onStashAndRetry` provided. `use-git-actions.ts` adds `stashAndRetryPull()` — three-step atomic sequence: stash all → retry pull → auto-pop on success. Pop conflict triggers informative toast and conflict panel via metadata polling. Pull failure after stash updates `gitActionError` with new error and `dirtyTree: false` (hides retry button). Wired through `App.tsx`.

**Tests**: `test/runtime/git-stash.test.ts` — 23 tests using real temp git repos (stash push/list/pop/apply/drop/show/count, conflict detection, partial stash, path validation, dirtyTree fields, edge cases). `test/runtime/trpc/workspace-api-stash.test.ts` — 9 tests with mocked git-sync (route wiring, broadcast verification, error handling). `test/runtime/git-sync-no-optional-locks.test.ts` — extended for `stashCount`. `web-ui/src/hooks/use-stash-list.test.ts` — 5 tests (fetch on expand, refetch on count change, action callbacks).

**Files touched**: `src/core/api-contract.ts`, `src/server/workspace-metadata-monitor.ts`, `src/trpc/app-router.ts`, `src/trpc/workspace-api.ts`, `src/workspace/git-sync.ts`, `web-ui/src/App.tsx`, `web-ui/src/components/app-toaster.ts`, `web-ui/src/components/card-detail-view.tsx`, `web-ui/src/components/detail-panels/checkout-confirmation-dialog.tsx`, `web-ui/src/components/detail-panels/commit-panel.tsx`, `web-ui/src/components/detail-panels/stash-list-section.tsx` (new), `web-ui/src/components/git-action-error-dialog.tsx`, `web-ui/src/hooks/use-branch-actions.ts`, `web-ui/src/hooks/use-commit-panel.ts`, `web-ui/src/hooks/use-git-actions.ts`, `web-ui/src/hooks/use-stash-list.ts` (new), `web-ui/src/stores/workspace-metadata-store.ts`, plus 4 test files.

## Feat: unified git push + merge conflict UX overhaul (2026-04-12)

Unified three feature branches into a single commit: ahead/behind indicators + push to remote, Commit & Push button, and merge conflict UX overhaul with auto-merged file review.

**Ahead/behind + push to remote**: `BranchPillTrigger` in `branch-selector-popover.tsx` now accepts optional `aheadCount`/`behindCount` props, rendering ↑N/↓N arrows from `ArrowUp`/`ArrowDown` lucide icons. Counts come from `parseAheadBehindCounts()` in `git-sync.ts`, parsing the `# branch.ab` line from `git status --porcelain=v2 --branch` (already run by the git probe — no extra commands). "Push to remote" context menu item on the current branch in `BranchSelectorPopover`, with a disabled-but-tooltipped variant for detached HEAD using `onSelect + preventDefault` instead of Radix `disabled` (to preserve tooltip pointer events). Task-scoped push adds `taskScope` to `runGitSyncAction` input in `app-router.ts`, resolving the worktree directory via `resolveTaskWorkingDirectory` in `workspace-api.ts`. `use-git-actions.ts` conditionally skips `setHomeGitSummary` for task-scoped pushes.

**Commit & Push button**: `use-commit-panel.ts` refactored from a single `commitFiles` handler to a shared `doCommit(pushAfterCommit)` function. `isOnNamedBranch` computed from `useTaskWorkspaceSnapshotValue` (task) or `useHomeGitSummaryValue` (home). `canPush = canCommit && isOnNamedBranch`. Backend in `workspace-api.ts` `commitSelectedFiles` checks `pushAfterCommit` flag and chains `runGitSyncAction({ cwd, action: "push" })` after successful commit, merging push result into response. `commit-panel.tsx` renders "Commit & Push" button with tooltip for detached HEAD state. Toast differentiates success ("Committed (hash) and pushed"), partial failure ("Committed (hash) but push failed: ..."), and commit-only. Schema additions: `pushAfterCommit` on request, `pushOk`/`pushError` on response.

**Merge conflict UX overhaul**: `runGitMergeAction` in `git-sync.ts` now uses `--no-commit --no-edit` to pause before auto-committing, enabling detection of auto-merged files via `computeAutoMergedFiles()` (runs `git diff --cached --name-only`, filters out conflicted files). `autoMergedFiles: z.array(z.string()).default([])` added to `runtimeConflictStateSchema`. Metadata monitors (`workspace-metadata-monitor.ts`) compute auto-merged files for both home and task workspaces. New tRPC endpoint `getAutoMergedFiles` fetches content via `git show HEAD:<path>` (old) and `git show :0:<path>` (merged). `use-conflict-resolution.ts` adds effect to fetch auto-merged content and `acceptAutoMergedFile` callback. `conflict-resolution-panel.tsx` refactored: three-section file list (unresolved, resolved, auto-merged), "Complete Merge" gated on all conflicts resolved AND all auto-merged files reviewed. Fetch failure auto-accepts to prevent deadlock. New `ConflictBanner` component: persistent orange indicator outside git view with operation type, conflict count, and "Resolve" link. `use-branch-actions.ts` updated: toast says "opening resolver" and fires `onConflictDetected` callback for auto-navigation.

**Review fixes**: Added missing `onConflictDetected` to `topbarBranchActions` in `App.tsx` (toast promised auto-open but didn't deliver for topbar merges). Auto-merged file fetch failure now auto-accepts files instead of deadlocking. Extracted `hashLabel` in `use-commit-panel.ts` to eliminate triplicated toast formatting.

**Files touched**: `src/core/api-contract.ts`, `src/server/workspace-metadata-monitor.ts`, `src/trpc/app-router.ts`, `src/trpc/workspace-api.ts`, `src/workspace/git-sync.ts`, `web-ui/src/App.tsx`, `web-ui/src/components/card-detail-view.tsx`, `web-ui/src/components/conflict-banner.tsx` (new), `web-ui/src/components/detail-panels/branch-selector-popover.tsx`, `web-ui/src/components/detail-panels/commit-panel.tsx`, `web-ui/src/components/detail-panels/conflict-resolution-panel.tsx`, `web-ui/src/components/git-view.tsx`, `web-ui/src/hooks/use-branch-actions.ts`, `web-ui/src/hooks/use-commit-panel.ts`, `web-ui/src/hooks/use-conflict-resolution.ts`, `web-ui/src/hooks/use-git-actions.ts`, `web-ui/src/stores/workspace-metadata-store.ts`, tests across 5 files.
