...
## Docs cleanup and version bump to 0.12.0 (2026-04-29)

Archived the 0.11.0 changelog section to `docs/history/changelog-through-0.11.0.md`, rotated the active implementation log into `docs/history/implementation-log-through-0.12.0.md`, bumped root package metadata from 0.11.0 to 0.12.0, and reset the active release notes for the next development cycle.

Files touched: `CHANGELOG.md`, `docs/implementation-log.md`, `docs/history/changelog-through-0.11.0.md`, `docs/history/implementation-log-through-0.12.0.md`, `package.json`, `package-lock.json`.
...
The picker now wraps `spawn` in an async result shape and the `projects.pickDirectory` mutation awaits it. Agent availability now runs Codex probes through async `execFile`, dedupes concurrent probes per agent/binary, preserves the 30-second TTL cache, and serves stale cached results while scheduling a background refresh after TTL expiry. Runtime config reads, startup resume, and task starts now await the async command resolver while retaining stale-while-revalidate behavior; settings save validation forces a fresh availability result so a newly installed or upgraded selected agent is not rejected once by stale cache data. Project registry creation and stream pruning use async git repo checks; indexed project validation runs through a bounded `p-limit` policy before sequentially removing invalid entries. Orphan-agent cleanup now reads `ps` through async `execFile`, exposes injectable discovery for tests, startup schedules cleanup with a zero-delay timer so boot can continue before scanning, and shutdown awaits orphan cleanup after closing the server so graceful process exit does not happen before async discovery can signal orphans.

Validation included `npm run typecheck`, `npx @biomejs/biome check --write` over the touched runtime/test files, `npx @biomejs/biome check` over those same code/test paths, `npx vitest run test/runtime/directory-picker.test.ts test/runtime/config/agent-registry.test.ts test/runtime/server/project-registry-startup-resume.test.ts test/runtime/terminal/orphan-cleanup.test.ts`, `npx vitest run test/runtime/config/agent-selection.test.ts test/runtime/config/config-persistence.test.ts test/runtime/trpc/runtime-api.test.ts`, `npx vitest run test/runtime/shutdown-coordinator-timeout.test.ts test/runtime/config/agent-selection.test.ts`, `npx vitest run test/integration/project-discovery.integration.test.ts`, and `npx vitest run test/integration/project-management.integration.test.ts`.

Files touched: `CHANGELOG.md`, `docs/implementation-log.md`, `src/cli.ts`, `src/config/agent-registry.ts`, `src/config/runtime-config.ts`, `src/server/directory-picker.ts`, `src/server/project-registry.ts`, `src/server/runtime-server.ts`, `src/server/shutdown-coordinator.ts`, `src/terminal/orphan-cleanup.ts`, `src/trpc/handlers/load-config.ts`, `src/trpc/handlers/save-config.ts`, `src/trpc/handlers/start-task-session.ts`, `src/trpc/projects-api.ts`, `test/runtime/config/agent-registry.test.ts`, `test/runtime/config/agent-selection.test.ts`, `test/runtime/directory-picker.test.ts`, `test/runtime/server/project-registry-startup-resume.test.ts`, `test/runtime/shutdown-coordinator-timeout.test.ts`, `test/runtime/terminal/orphan-cleanup.test.ts`.
...
Archived 7 completed refactor docs to `docs/history/`, rotated changelog and implementation log into their archive files, bumped version from 0.10.0 to 0.11.0, and updated `docs/README.md` cross-references.
