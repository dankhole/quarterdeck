...
- Dogfood Claude Code's fullscreen renderer as the durable redraw/scrollback fix. Start behind an opt-in Quarterdeck setting, inject `CLAUDE_CODE_NO_FLICKER=1`, disable Claude's detached row multiplier only for fullscreen sessions, and validate alternate-screen restore plus xterm mouse/selection behavior before considering a default-on rollout. Plan: [docs/claude-terminal-rendering-plan.md](./claude-terminal-rendering-plan.md).
