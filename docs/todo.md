...
- ~~**Stale-while-revalidate**: Cache board state per project in memory. On switch, show the cached version immediately while fresh data loads. Requires careful gating of `canPersistWorkspaceState` and `workspaceRevision` to prevent stale data from being persisted back to disk.~~ **Done.** Module-scoped `project-board-cache.ts` stashes board/sessions/revision/workspace metadata per project (5-minute TTL, max 10 entries). On project switch, the previous board is stashed and the target board is restored from cache immediately — the loading spinner is suppressed and the cached board is displayed while fresh data loads in the background. `canPersistWorkspaceState` stays `false` until authoritative data arrives, preventing stale writes. Complements the existing preload-on-hover cache (first visits) with long-lived caching (revisits).
