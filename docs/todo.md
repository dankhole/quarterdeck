...
- Complete Agent Lab headless workflow parity for host-only actions. Keep native UI fail-closed, but add browser-manageable substitutes or explicit observable lab fakes for Open in IDE, host file/folder and URL opening, clipboard, audio/notifications, and future audited integrations. Audit for UI flows whose only completion path crosses host state so agents can exercise and assert both blocked and simulated-success behavior without touching the user's desktop.
