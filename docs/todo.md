...
- Refactor lightweight LLM generation as a two-phase effort. Phase 1: turn `src/title/llm-client.ts` into a provider-neutral lightweight-generation client instead of a Bedrock/Anthropic-specific helper, while preserving existing title, summary, branch-name, and commit-message behavior. Phase 2: improve generated task titles and display summaries by reviewing prompts, source context, trigger timing, fallback behavior, stale-regeneration behavior, and editability so generated text is more specific and useful on the board without requiring manual cleanup. Keep the phases separately testable and preserve graceful fallback when LLM generation is unavailable. Backlog context: [docs/architecture-roadmap.md#12-shared-llm-client-abstraction](./architecture-roadmap.md#12-shared-llm-client-abstraction)
