...
- Continue the behavior-preserving foundations for the text-only Remote Companion after the completed runtime board-command cutover: add provider-neutral bounded conversation reads for Claude/Codex, structured non-PTY task interaction, strict leak-tested remote projections, and a browserless acceptance harness before exposing any remote listener. Keep native agent TUIs and hooks as the existing local interaction/lifecycle authorities. Plan: [docs/remote-companion-plan.md](./remote-companion-plan.md).
