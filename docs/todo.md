...
## 5. Cherry-pick / land individual commits onto main from the UI
...
This is distinct from #11 (committing *within* the task worktree) and #4 (full git management with branch merging). This is a targeted "cherry-pick to main" action, likely surfaced as a button in the diff viewer or on the task card during review.

## 6. Upstream sync: periodic review of cline/kanban (recurring)
...
## 7. Audit CI/CD and deployment infrastructure
...
## 8. Investigate auto-trashing of tasks on restart
...
## 9. Investigate un-trash / restart paths for non-isolated worktrees
...
## 12. Interactive base ref switcher
...
## 13. Fix: notification beep count wrong for rapid state transitions

When a task goes to "ready for review" then quickly switches to "needs input", only 1 beep plays instead of 2. Also, "waiting for approval" may always be playing only 1 beep regardless of config. This may overlap with #14 (double-beep / missed cues) — check the implementation log, as a recent fix may have partially addressed this.

## 14. Fix: audible notification double-beep and missed cues
...
## 17. Decouple session summary dual-sourcing between terminal and state layers
...
## 18. Archive stale docs (recurring)
...
## 19. Move agent chat out of the project switcher sidebar
...
## 20. Slow project switching — cache or preload board state
...
## 21. Independent sidebar widths per panel type (post diff rewrite)
...
## 22. Un-trashing shared-workspace tasks clobbers session state
...
## 23. Un-trash doesn't always auto-resume the agent session
...
## 24. Task stuck in "running" when agent is waiting for permission
