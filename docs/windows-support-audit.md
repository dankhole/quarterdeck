...
- **Symlink strategy:** Directory junctions mirror eligible ignored directories on Windows, but root and nested `node_modules` paths are always excluded because installed dependency trees are mutable. Mutable build-output exclusions also avoid known `.NET` paths. The broader ignored-path allowlist follow-up remains active and matters for Windows because junction mirroring can cross worktree isolation boundaries.
