...
| `9398a457` | Plan mode via prompt injection instead of SDK mode | Already had | Upstream replaced their SDK `mode: "plan"` with a prompt prefix instructing the agent to plan without modifying files. Quarterdeck already does this via `startInPlanMode` + `append-system-prompt.ts`. Upstream converged on our approach. |
...
`2875041c` (skip Cline session-host probe on Codex trash restore), `577009f4` (friendly labels for Cline task tool calls in sidebar chat), `e9a8f1b5` (nullable fetchRemoteConfig in SDK boundary), `95f2edaf` (stream clear semantics in Cline message repository), `9398a457` (plan mode — adopted concept, not code), `83f750b7` (trash resume — adopted concept, not code)
