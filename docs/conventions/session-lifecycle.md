...
Spawning a fresh, resumed, or replacement native PTY proves only that its input surface exists. Seed it as Review/Unconfirmed (or preserve the stronger durable Review/interaction meaning being restored), never Running. The first current launch-scoped foreground-start hook (`UserPromptSubmit` for Codex/Claude or `Input`/`AgentStart` for Pi) immediately establishes Running; a current `PreToolUse` remains the fallback when the start hook is missing. A reliable state hook that reaches the runtime before the exact PTY completes process-ownership handoff remains unacknowledged so its durable outbox record retries after handoff; never commit its delivery order as an unconfirmed no-op. That admitted foreground execution stays Running through hook silence until a typed root completion, interaction, interruption, process exit, replacement, or recovery event ends or suspends it. `SessionStart` remains metadata-only because resume, compaction, and maintenance can emit it without starting foreground work. Cold hydration invalidates the previous runtime's evidence before startup recovery. Wall-clock silence, terminal output, and process liveness are health observations, never lifecycle transitions.
