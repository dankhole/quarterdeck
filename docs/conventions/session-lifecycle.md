...
- The controller is the sole author of live native-work confirmation and its launch-scoped `nativeWorkEvidence`; callers cannot assert either. A raw provider event and its metadata are one session-store mutation through `applySessionEvent(...)`, which atomically applies state, review reason, outstanding interaction, work evidence, activity, resume identity, and timestamps. Do not restore compatibility methods with separate replay or error semantics.
- Claude launch permissions are provider-native modes, not Codex reviewer aliases. `inherit` leaves Claude's configuration and supplied arguments untouched; every explicit Quarterdeck mode removes conflicting `--permission-mode`, dangerous-skip, and dangerous-mode-enablement arguments before inserting exactly one launch-scoped `--permission-mode`. The frozen request carries that choice through automatic replacement and startup recovery, while Claude-managed policy remains authoritative.
...
An explicit fresh Start seeds Running after successful PTY ownership handoff, without fabricating native work evidence. Its launch-scoped 45-second work-confirmation deadline moves a still-unconfirmed launch to Review/Unconfirmed while leaving the process available. Metadata-only SessionStart does not cancel this deadline; accepted work, completion, interaction, interruption, exit, or teardown ends the initial pending phase. An exit before confirmation becomes Error and cannot trigger automatic crash recovery. A launch that already produced native work and later returns to Review/Unconfirmed (for example after a Codex conversation switch) is not a launch failure: exit code 0 records Exit. Resumed and replacement native PTYs still seed Review/Unconfirmed (or preserve stronger durable Review/interaction meaning). The first current launch-scoped foreground-start hook (`UserPromptSubmit` for Codex/Claude or `Input`/`AgentStart` for Pi) immediately establishes Running; a current `PreToolUse` remains the fallback when the start hook is missing. A reliable state hook that reaches the runtime before the exact PTY completes process-ownership handoff remains unacknowledged so its durable outbox record retries after handoff; never commit its delivery order as an unconfirmed no-op. That admitted foreground execution stays Running through hook silence until a typed root completion, interaction, interruption, process exit, replacement, or recovery event ends or suspends it. `SessionStart` remains metadata-only because resume, compaction, and maintenance can emit it without starting foreground work. Cold hydration invalidates the previous runtime's evidence before startup recovery. After native work is confirmed, wall-clock silence, terminal output, and process liveness are health observations, never lifecycle transitions.

Bare Escape is forwarded to the provider without changing Running, suppressing restart, or fencing queued hooks; it may only record cancellation of an existing actionable input wait. Provider interruption evidence owns any resulting Interrupted transition. Ctrl-C synchronously applies Review/Interrupted before its bounded process-exit recovery bookkeeping begins. The later timer retires only pending process-exit recovery bookkeeping; the launch-scoped latest-interrupt timestamp survives timer cleanup and resumed work so delayed pre-interrupt hooks remain rejected. A current post-interrupt working hook may establish Running again, and a current completion hook may converge directly to ordinary Review.
...
Worktree registration changes and submodule initialization share the repository setup lock. Creation-time copying and scripts use a stable per-worktree operation lock in the Git common directory; archive and purge take that same lock before the repository lock. This prevents removal under a running setup without holding the repository lock for a dependency install. Lock targets must be distinct, not just different lockfile names on one target: proper-lockfile tracks ownership by target path. Missing or mismatched registration fails explicitly and preserves existing task files; never delete and recreate a broken existing folder as automatic recovery. Repair requires independent evidence of the correct registration and preserved task work.

Creation marks setup pending in the worktree's Git administrative directory before restoring saved task changes. The setup owner initializes submodules, copies `.worktreeinclude` selections, runs the project-configured script, then records success. Failed/interrupted setup retains files and blocks all launch paths, including persisted working directories. Explicit lifecycle Start/Restart may retry; automatic recovery and replay may not. Successful and legacy unmarked worktrees never rerun creation-time scripts. Setup progress is a lifecycle-operation phase, not a new agent session state. See [worktree setup](../worktree-setup.md) for configuration and user-facing behavior.
...
- Interrupt recovery and an explicit PTY stop can both surface as `awaiting_review/interrupted`, but they are not the same process state. A pending bare Ctrl-C/Escape recovery signal owns an immediate PTY exit even when the process reports code 0; persist Interrupted and suppress auto-restart instead of inferring completion from the exit code. Only an accepted current provider hook carrying that exact PTY's `sessionInstanceId` and a delivery time after the latest interrupt, while `PtySession.wasInterrupted()` is false, may advance or complete the review. Either accepted semantic proof cancels the interrupt timer and clears its one-shot auto-restart suppression, including activity-style `PreToolUse`; permission and notification no-ops cannot do so. Rejected completions cannot publish summary metadata through ingest. Once an explicit stop marks the PTY interrupted, reject further input and provider evidence until a replacement session starts.

### Native terminal input authorization

Terminal IO uses `createNativeTerminalInputWriter`, scoped to one WebSocket and one exact native PTY. Its first ownership check reads the durable journal; subsequent input checks the journal's file identity asynchronously and reuses the store's subscribed observation while that identity is unchanged, without taking the project disk lock or parsing the journal again. `ProjectExecutionOwnershipStore` publishes verified reads and successful commits, invalidates changed ownership before a write, and leaves failed verification unknown. Unrelated task and interaction writes do not revoke a verified observation. After every await, the writer rechecks the exact PTY and current observation, then checks structured ownership and writes synchronously under the task resource coordinator. Disconnect disposes the subscription and prevents queued writes. An attachment made before launch may bind on its first input to an already-existing PTY; bytes received before that PTY exists are discarded, and a bound connection never follows a replacement PTY.

This observation is an input optimization, not lifecycle authority. Start, handoff, and recovery retain durable checks. Multiple runtimes may share a state home, so local publication alone cannot prove freshness: each input compares device/inode/size and nanosecond modification/change timestamps with the token captured under the durable journal lock. External replacement or metadata-check failure invalidates the observation; external changes require durable reverification before writing. Subscription versions prevent a stale asynchronous metadata result from overriding a newer local publication. Backup restore still requires stopped runtimes.
...
- Preserve the Codex 0.157.0 minimum-version and native-hooks feature gate. Only genuine user-input or approval waits surface as Needs Input.
...
- Codex's native `Interrupt` hook is the primary turn-abort signal. Install it with launch-scoped trust, reliable ingest, no matcher, and a three-second timeout (the provider's maximum). Require its main-session and turn identity; apply Review/Interrupted, clear the foreground interaction, activity, and work evidence, and retain exact resume identity without completion summaries, checkpoints, notifications, or sounds. Its durable receipt closes the turn against delayed permission, tool, and Stop hooks; a newer foreground turn may establish Running. A valid pre-exit Interrupt replay can resolve an interaction made `resolution_unknown` by process loss.
- The [documented Interrupt contract](https://learn.chatgpt.com/docs/hooks#interrupt) covers user interruption of an active main-thread turn, not general failure reasons, idle threads, or subagents. Retain the bounded rendered "Conversation interrupted" fallback for hookless failures outside that contract, including foreground permission cancellation without a native terminal hook. It is terminal negative evidence only; output never establishes Running. Do not infer failure causes or claim full wrapper/parser failure granularity from Interrupt.
...
Persistent Codex session navigation retires the previous session's outstanding interaction and work evidence into Review/Unconfirmed while updating resume identity and ordering ownership together. SessionStart alone never proves new work; the new session's native working hook must establish Running.
