...
Read this before changing or integrating non-PTY task execution, native/structured handoff, remote task interaction, provider session ownership, or provider-history compatibility.

This is the stable contract distilled from the completed provider-boundary and ownership-handoff spikes. Implementation status and experimental evidence remain in the linked decision records; ordinary changes should start here rather than loading every spike transcript.
...
- Remote code calls `ProjectBoardCommandService`, `ProjectTaskLifecycleService`, and the provider-owned interaction boundary directly. It does not reuse browser presentation adapters or raw PTY input.
...
- Concurrent Claude or background-agent remote interaction remains unsupported until it has a durable keyed model. Do not route subagent/global Agent View notifications through the foreground record.
- Pi structured or remote compatibility is out of scope until its ownership and exact-resume contract is designed and validated explicitly.
...
## Detailed evidence and active work

- [Conversation provider boundary spike](../conversation-provider-boundary-spike.md)
- [Remote task ownership handoff results](../remote-task-ownership-handoff-spike-results.md)
- [Remote Companion plan](../remote-companion-plan.md)
