...
Initial title generation is triggered by the board command service's post-commit `untitled_task_created` effect. This is the only automatic title trigger. Completing a turn never regenerates a title; follow-up generation requires the user's explicit Auto-generate title action.
...
- Initial and follow-up prompts favor short, recognizable subject/purpose labels, such as “UI Work” or “Kafka Investigation,” while retaining useful distinguishing subjects. Avoid first-step titles, subtask lists, and incidental completion details. Explicit regeneration uses the conversation's current direction.
- `titleAutoGenerated` is server-owned provenance, not an automatic-refresh opt-in. Initial/generated titles set it to true; explicit title edits set it to false, including a same-wording rename. Clients cannot supply provenance through board commands. Existing titles stay unchanged unless explicitly edited or regenerated.
