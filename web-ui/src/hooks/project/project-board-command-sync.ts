...
function hasManualTitleChange(before: BoardCard, after: BoardCard): boolean {
	return before.title !== after.title || (before.titleAutoGenerated === true && after.titleAutoGenerated === false);
}

function hasEditableTaskChange(before: BoardCard, after: BoardCard): boolean {
	return (
		hasManualTitleChange(before, after) ||
	...
	for (const [taskId, nextLocation] of afterCards) {
		...
		if (hasEditableTaskChange(previousLocation.card, nextLocation.card)) {
			commands.push({
				kind: "update_task",
				taskId,
				...(hasManualTitleChange(previousLocation.card, nextLocation.card)
