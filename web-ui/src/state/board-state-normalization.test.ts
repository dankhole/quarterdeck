...
describe("normalizeBoardData", () => {
	it("merges legacy Backlog into Review while preserving started Review tasks", () => {
		...
		const board = normalizeBoardData({
			columns: [
				{ id: "backlog", cards: [{ ...task, id: "queued" }] },
				{
					id: "review",
					cards: [
						{ ...task, id: "finished" },
						{ ...task, id: "new", unstarted: true },
					],
				},
			],
		});
		expect(board?.columns.map((column) => column.id)).toEqual(["in_progress", "review", "trash"]);
		const cards = board?.columns.find((column) => column.id === "review")?.cards;
		expect(cards?.map((card) => [card.id, card.unstarted === true])).toEqual([
			["queued", true],
			["finished", false],
			["new", true],
		]);
	...
	it("retains title ownership during authoritative board hydration without inventing legacy ownership", () => {
		const board = normalizeBoardData({
			columns: [
				{
					id: "backlog",
					cards: [true, false, undefined, "true"].map((titleAutoGenerated, index) => ({
						id: `task-${index}`,
		...
		expect(
			board?.columns.find((column) => column.id === "review")?.cards.map((card) => card.titleAutoGenerated),
	...
	it("creates tasks when randomUUID is unavailable", () => {
		...
		const board = addTaskToColumn(createInitialBoardData(), "review", {
			prompt: "Task A",
			baseRef: "main",
		});
		const unstartedCards = board.columns.find((column) => column.id === "review")?.cards ?? [];

		expect(unstartedCards).toHaveLength(1);
		expect(unstartedCards[0]?.id).toHaveLength(5);
	});
	...
	it("uses random entropy when randomUUID is unavailable", () => {
		...
		expect(unstartedCards[0]?.id).toBe("44444");
	});
	...
	it("keeps fallback task IDs fixed-length when random returns zero", () => {
		...
		expect(unstartedCards[0]?.id).toBe("00000");
		expect(unstartedCards[0]?.id).toHaveLength(5);
	});
